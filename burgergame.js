...
function renderLoginButton() {
	// register the google signin button
	gapi.signin.render('customGoogleButton', {
		callback: 'signinCallback',
		clientid: '171967825850.apps.googleusercontent.com',
...
function signinCallback(authResult) {
	if (authResult['access_token']) {
		...
		//$("#loginsBox").css("display", "none");

		if (mechanics.userDebug) {
			console.log("google plus signin successful!");
		}

		player.access_token = authResult['access_token'];
		...
		gapi.client.load('plus','v1', function(){
			...
			request.execute(function(resp) {
				...
				player._id = "G" + resp.id;

				player.load();

				$("#logoutBox")
					.click(function() {
						disconnectUser(player.access_token);	
					})
					.toggle();
...
$(document).ready(function() {
	...
	$("#customFBButton").click(function() {
		console.log("clicking the fb button!");
		fbAuth();
	});
